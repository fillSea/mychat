...
ChatUserList::ChatUserList(QWidget *parent) : QListWidget(parent), load_pending_(false) {
	Q_UNUSED(parent);
	// 禁用水平滚动条
...
void ChatUserList::SlotItemClicked(QListWidgetItem* item) {
	// 获取被点击 item 对应的 ChatUserItem 控件
	ChatUserItem* chatUserItem =
		static_cast<ChatUserItem*>(itemWidget(item));
	if (chatUserItem) {
		// 这里可以获取用户信息并进行相应处理
		// 例如显示聊天窗口、切换会话等
		qDebug() << "点击了用户: " << chatUserItem->GetUserInfo()->name_;
		std::cout << u8"点击了用户" << std::endl;
